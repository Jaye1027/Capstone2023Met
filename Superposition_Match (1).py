...
for i in range(dates_length):
    current_date_storm = storm_dates[i] # set current date for events
    for j in range(super_length):
        current_date_jet = super_file['Dates'][j] # set current date for jets
        if current_date_jet == current_date_storm: # are dates equal?
            equal_df.append(current_date_jet) # add to array
            distance = super_file[j][lat] - events_file[i][lon]
            print(distance)
